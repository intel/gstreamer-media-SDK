...
GstMfxSurface *
gst_mfx_surface_vaapi_new_external (GstMfxContext * context,
...
GstMfxSurface *
gst_mfx_surface_vaapi_new_from_buffer_proxy (GstMfxContext * context,
    GstMfxPrimeBufferProxy * proxy, const GstVideoInfo * info)
{
  ...
  surface =
      (GstMfxSurface *) gst_mfx_surface_vaapi_new_external (context, info);
  ...
  GST_MFX_DISPLAY_LOCK (vaapi_surface->display);
  sts = vaCreateSurfaces (GST_MFX_DISPLAY_VADISPLAY (vaapi_surface->display),
      gst_mfx_video_format_to_va_format (frame_info->FourCC), width, height,
      (VASurfaceID *) & priv->surface_id, 1, (VASurfaceAttrib *) & attribs, 2);
  GST_MFX_DISPLAY_UNLOCK (vaapi_surface->display);
...
GstMfxSurface *
gst_mfx_surface_vaapi_new_with_dma_buf_handle (GstMfxContext * context, gint fd,
    GstVideoInfo * vi)
{
  ...
  gst_mfx_prime_buffer_proxy_set (proxy, fd, vi, context);

  return gst_mfx_surface_vaapi_new_from_buffer_proxy (context, proxy, vi);
}
