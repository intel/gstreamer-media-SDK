...
gst_mfx_profile_from_codec_data_h264(GstBuffer *buffer)
{
	...
	case 77:    return GST_MFX_PROFILE_AVC_MAIN;
	case 100:   return GST_MFX_PROFILE_AVC_HIGH;
...
gst_mfx_profile_from_codec_data_h265(GstBuffer *buffer)
{
	...
	case 1:   return GST_MFX_PROFILE_HEVC_MAIN;
	case 2:   return GST_MFX_PROFILE_HEVC_MAIN10;
	case 3:   return GST_MFX_PROFILE_HEVC_MAIN_STILL_PICTURE;
	}
	return 0;
}
...
gst_mfx_profile_from_caps(const GstCaps *caps)
{
	...
	for (m = gst_mfx_profiles; !profile && m->profile; m++) {
		...
		if (!profile) {
			profile = gst_mfx_profile_from_codec_data(
				gst_mfx_profile_get_codec(m->profile),
				codec_data
				);
		}
