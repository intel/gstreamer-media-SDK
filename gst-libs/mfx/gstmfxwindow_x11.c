...
{
	GstMfxWindowX11Private *const priv =
...
static gboolean
timed_wait_event(GstMfxWindow * window, int type, guint64 end_time,
	XEvent * e)
{
	Display *const dpy = GST_MFX_OBJECT_NATIVE_DISPLAY(window);
	const Window xid = GST_MFX_OBJECT_ID(window);
...
static gboolean
gst_mfx_window_x11_create(GstMfxWindow * window, guint * width,
	guint * height)
{
	GstMfxWindowX11Private *const priv =
		GST_MFX_WINDOW_X11_GET_PRIVATE(window);
...
static gboolean
gst_mfx_window_x11_set_fullscreen(GstMfxWindow * window,
	gboolean fullscreen)
{
	GstMfxWindowX11Private *const priv =
		GST_MFX_WINDOW_X11_GET_PRIVATE(window);
