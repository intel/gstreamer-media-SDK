    ...
    guint prop_id, const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_TS_OFFSET:
      g_object_set (G_OBJECT (mfxsinkbin->sink),
          pspec->name, g_value_get_int64 (value), NULL);
      break;
    case PROP_BLOCKSIZE:
      g_object_set (G_OBJECT (mfxsinkbin->sink),
          pspec->name, g_value_get_uint (value), NULL);
      break;
    case PROP_RENDER_DELAY:
    case PROP_THROTTLE_TIME:
    case PROP_MAX_BITRATE:
      g_object_set (G_OBJECT (mfxsinkbin->sink),
          pspec->name, g_value_get_uint64 (value), NULL);
      break;
    ...
    case PROP_SHOW_PREROLL_FRAME:
      g_object_set (G_OBJECT (mfxsinkbin->sink),
          pspec->name, g_value_get_boolean (value), NULL);
      break;
    ...
    case PROP_CONTRAST:
      g_object_set (G_OBJECT (mfxsinkbin->postproc),
          pspec->name, g_value_get_float (value), NULL);
      break;
    default:
    ...
    guint prop_id, GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_SHOW_PREROLL_FRAME:
      if (mfxsinkbin->sink) {
        g_object_get_property (G_OBJECT (mfxsinkbin->sink), pspec->name, value);
      }
      break;
      /* VPP */
    case PROP_WIDTH:
...
gst_mfx_sink_bin_class_init (GstMfxSinkBinClass * klass)
{
  ...
  gst_element_class_set_static_metadata (element_class,
      "MFX Sink Bin",
      "Sink/Video",
      GST_PLUGIN_DESC, "Puunithaaraj Gopal <puunithaaraj.gopal@intel.com>");
  ...
  g_properties[PROP_DEINTERLACE_METHOD] =
      ...
      DEFAULT_DEINTERLACE_METHOD, G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
...
gst_mfx_sink_bin_configure (GstMfxSinkBin * mfxsinkbin)
{
  ...
  gst_bin_add_many (GST_BIN (mfxsinkbin),
      mfxsinkbin->postproc, mfxsinkbin->sink, NULL);

  if (!gst_element_link_many (mfxsinkbin->postproc, mfxsinkbin->sink, NULL))
    goto error_link_pad;

  /* ghost pad sink */
  pad = gst_element_get_static_pad (GST_ELEMENT (mfxsinkbin->postproc), "sink");
...
error_adding_pad:
  {
    GST_ELEMENT_ERROR (mfxsinkbin, CORE, PAD, (NULL), ("Failed to add pads."));
    return FALSE;
  }
}
